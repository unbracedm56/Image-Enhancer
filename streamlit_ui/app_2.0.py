...
def process_style_transfer(vgg_model, device, content_image, style_image, total_steps, learning_rate, alpha, beta):
    ...
    # The style transfer process resizes to 356x356 internally
    image_size = 356
...
# --- UI for Neural Style Transfer ---
    ...
    st.info("Transfer the style of one image onto the content of another. Images will be internally resized to **356x356**.")
    ...
    if content_file is not None and style_file is not None:
        ...
        st.subheader("Your Images (Resized to 356x356 for Processing)")
        img_col1, img_col2 = st.columns(2)
        with img_col1:
            st.image(content_image, caption="Content Image", use_container_width=True) # WARNING FIX
        with img_col2:
            st.image(style_image, caption="Style Image", use_container_width=True) # WARNING FIX
