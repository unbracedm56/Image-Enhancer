...
if model_choice == "Image Colorization":
    ...
    st.info("Upload a black and white image to add color to it using a U-Net Generator.")
    ...
    if uploaded_file is not None:
        ...
        if st.button("🎨 Colorize Image"):
            ...
            with st.spinner("Applying color..."):
                colorized_image = process_colorization(model, device, original_image)
            with col2:
...
# --- UI for Image Brightening ---
    ...
    st.info("Upload a dark or underexposed image to brighten it using a GAN.")
    ...
    if uploaded_file is not None:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.image(original_image, caption="Original Dark Image", use_column_width=True)
        if st.button("💡 Brighten Image"):
            model = get_brightening_model()
            with st.spinner("Illuminating image..."):
                brightened_image = process_brightening(model, original_image)
            with col2:
...
# --- UI for Neural Style Transfer ---
    ...
    st.info("Transfer the style of one image onto the content of another using a VGG-19 model.")
    ...
    if content_file is not None and style_file is not None:
        ...
        if st.button("🚀 Start Style Transfer"):
            ...
            st.image(generated_image, caption="Final Generated Image", use_column_width=True)

# --- UI for Image Denoising ---
elif model_choice == "Image Denoising":
    st.header("Image Denoising")
    st.info("Upload a noisy image to remove noise using the SwinIR model. This can be slow on large images or without a GPU.")
    
    st.sidebar.markdown("### Denoising Performance")
    resize_for_speed = st.sidebar.checkbox("Resize image for faster processing", value=True)
    resize_percentage = 50
    if resize_for_speed:
        resize_percentage = st.sidebar.slider("Resize image to (% of original)", 25, 100, 50)
    ...
    if uploaded_file is not None:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.image(original_image, caption="Original Noisy Image", use_column_width=True)
        ...
        if st.button("🧹 Denoise Image"):
            ...
            if resize_for_speed:
                ...
                st.info(f"Resizing image to {new_width}x{new_height} for faster processing...")
                image_to_process = image_to_process.resize((new_width, new_height), Image.Resampling.LANCZOS)
...
# --- UI for Image Upscaling (MODIFIED) ---
    ...
    st.info("Upload a low-resolution image to increase its quality and size (2x) using an RRDB-based model.")
    st.warning("Please wait, this process can take a long time, especially for large images or on a CPU.")
    ...
    if uploaded_file is not None:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.image(original_image, caption=f"Original Image ({original_image.width}x{original_image.height})", use_column_width=True)

        if st.button("🚀 Upscale Image"):
            model, device = get_upscaler_model()
            with st.spinner("Upscaling image... Please be patient, this will take a while."):
                upscaled_image = process_upscaling(model, device, original_image)

            with col2:
                st.image(upscaled_image, caption=f"Upscaled Image ({upscaled_image.width}x{upscaled_image.height})", use_column_width=True)